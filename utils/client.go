...
func NewClient(url, key string, options *ClientOptions) (*Client, error) {
	...
	client.Rest = postgrest.NewClient(url+REST_ENDPOINT, schema, headers)
	client.Storage = storage_go.NewClient(url+STORAGE_ENDPOINT, key, headers)
	tmp := gotrue.New(url, key)
	client.Auth = tmp.WithCustomGoTrueURL(url + AUTH_ENDPOINT)
	client.Functions = functions.NewClient(url+FUNCTIONS_ENDPOINT, key, headers)
...
func (c *Client) UpdateAuthSession(session types.Session) {
	...
	c.Storage = storage_go.NewClient(c.options.url+STORAGE_ENDPOINT, session.AccessToken, c.options.headers)
	c.Functions = functions.NewClient(c.options.url+FUNCTIONS_ENDPOINT, session.AccessToken, c.options.headers)
