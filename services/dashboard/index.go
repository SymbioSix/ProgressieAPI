...
func (dash *DashboardController) SidebarMapper(c fiber.Ctx) error {
	...
	for i := 0; i < len(userRoleResponse.RoleData); i++ {
		if getRoleSidebarFromAuthenticatedUser := dash.DB.Table("usr_rolesidebar").Preload(clause.Associations).Find(&roleSidebarResponse, "role_id = ?", userRoleResponse.RoleData[i]); getRoleSidebarFromAuthenticatedUser != nil {
